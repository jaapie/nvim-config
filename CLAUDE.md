...
| `oil.lua` | oil.nvim — default file explorer replacing netrw |
...
| `monokai-pro.lua` | Alternate colorscheme (not active — colorscheme cmd commented out) |

## LSP setup details

LSP keymaps are registered inside an `LspAttach` autocmd in `lsp.lua` (not globally). Sorbet is only enabled when `sorbet/config` exists in the project root. Diagnostics use `virtual_lines = { current_line = true }` (no virtual_text).
...
doorboy intercepts bare `(`, `{`, `[` in insert mode and inserts the closing pair immediately. The `{<cr>` keymap in `keymaps.lua` works around this for braces; the equivalent `(<cr>` and `[<cr>` mappings are commented out because doorboy interferes with them.
